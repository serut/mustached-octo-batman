<?php
// here is a place to enter general settings for all servers using this game

$list_of_gtypes[] = 'dm Deathmatch';
$list_of_gtypes[] = 'tdm Team Deathmatch';
$list_of_gtypes[] = 'sd Search and Destroy';
$list_of_gtypes[] = 'hq Headquarters';
$list_of_gtypes[] = 'ctf Capture the Flag';

$list_of_maps[] = 'mp_farmhouse Beltot, France';
$list_of_maps[] = 'mp_brecourt Brecourt, France';
$list_of_maps[] = 'mp_burgundy Burgundy, France';
$list_of_maps[] = 'mp_trainstation Caen, France';
$list_of_maps[] = 'mp_carentan Carentan, France';
$list_of_maps[] = 'mp_decoy El Alamein, Egypt';
$list_of_maps[] = 'mp_leningrad Leningrad, Russia';
$list_of_maps[] = 'mp_matmata Matmata, Tunisia';
$list_of_maps[] = 'mp_downtown Moscow, Russia';
$list_of_maps[] = 'mp_harbor Rostov, Russia'; //1.3
$list_of_maps[] = 'mp_dawnville St. Mere Eglise, France';
$list_of_maps[] = 'mp_railyard Stalingrad, Russia';
$list_of_maps[] = 'mp_toujane Toujane, Tunisia';
$list_of_maps[] = 'mp_breakout Villers-Bocage, France';
$list_of_maps[] = 'mp_rhine Wallendar, Germany'; //1.3
$list_of_maps[] = 'mp_naout North Africa Outpost'; //Custom
$list_of_maps[] = 'mp_africa Somewhere in Africa'; //Custom
$list_of_maps[] = 'mp_asylum Asylum'; //Custom
$list_of_maps[] = 'mp_chateau Chateau'; //Custom
$list_of_maps[] = 'mp_chateau_manor Manor'; //Custom
$list_of_maps[] = 'mp_complex_v2 The Complex v2'; //Custom
$list_of_maps[] = 'mp_harbor_v2 Harbor v2'; //Custom
$list_of_maps[] = 'mp_ht_factory Factory'; //Custom
$list_of_maps[] = 'mp_marais Chti-Marais v2'; //Custom
$list_of_maps[] = 'mp_mp Houffalize'; //Custom
$list_of_maps[] = 'mp_neuville Neuville'; //Custom
$list_of_maps[] = 'mp_tsl2011day The Short Line 2011 v1.2'; //Custom
$list_of_maps[] = 'mp_waldcamp Walscamp'; //Custom
$list_of_maps[] = 'mp_depot_winter Depot Winter'; //Custom
$list_of_maps[] = 'mp_bridge The Bridge'; //Custom
$list_of_maps[] = 'mp_buhlert Buhlert'; //Custom
$list_of_maps[] = 'mp_colditzfinal Colditz'; //Custom
$list_of_maps[] = 'mp_desertville Desert Ville'; //Custom
$list_of_maps[] = 'mp_maaloy_s Maaloy'; //Custom
$list_of_maps[] = 'mp_port Port'; //Custom
$list_of_maps[] = 'mp_Argentan_France Argentan'; //Custom
$list_of_maps[] = 'mp_arrigny_sm Arrigny'; //Custom
$list_of_maps[] = 'mp_brunville Brunville'; //Custom
$list_of_maps[] = 'mp_bzt Bazna'; //Custom

$list_of_weapons[] = 'scr_allow_fraggrenades';
$list_of_weapons[] = 'scr_allow_smokegrenades';
$list_of_weapons[] = 'scr_allow_shotgun';

$list_of_weapons[] = 'scr_allow_svt40';
$list_of_weapons[] = 'scr_allow_m1carbine';
$list_of_weapons[] = 'scr_allow_m1garand';
$list_of_weapons[] = 'scr_allow_g43';
$list_of_weapons[] = 'scr_allow_bar';
$list_of_weapons[] = 'scr_allow_bren';
$list_of_weapons[] = 'scr_allow_mp44';
$list_of_weapons[] = 'scr_allow_ppsh';
$list_of_weapons[] = 'scr_allow_sten';
$list_of_weapons[] = 'scr_allow_mp40';
$list_of_weapons[] = 'scr_allow_thompson';
$list_of_weapons[] = 'scr_allow_pps42';
$list_of_weapons[] = 'scr_allow_greasegun';
$list_of_weapons[] = 'scr_allow_kar98k';
$list_of_weapons[] = 'scr_allow_nagant';
$list_of_weapons[] = 'scr_allow_enfield';
$list_of_weapons[] = 'scr_allow_kar98ksniper';
$list_of_weapons[] = 'scr_allow_nagantsniper';
$list_of_weapons[] = 'scr_allow_enfieldsniper';
$list_of_weapons[] = 'scr_allow_springfield';

$lang['scr_allow_m1carbine'] = 'M1A1 Carbine';
$lang['scr_allow_m1garand'] = 'M1 Garand';
$lang['scr_allow_svt40'] = 'Tokarev SVT-40';
$lang['scr_allow_g43'] = 'Gewehr 43';
$lang['scr_allow_bar'] = 'BAR';
$lang['scr_allow_bren'] = 'Bren LMG';
$lang['scr_allow_mp44'] = 'MP44';
$lang['scr_allow_ppsh'] = 'PPSh';
$lang['scr_allow_sten'] = 'Sten';
$lang['scr_allow_mp40'] = 'MP40';
$lang['scr_allow_thompson'] = 'Thompson';
$lang['scr_allow_pps42'] = 'PPS42';
$lang['scr_allow_greasegun'] = 'Grease Gun';
$lang['scr_allow_kar98k'] = 'Kar98k';
$lang['scr_allow_nagant'] = 'Mosin-Nagant';
$lang['scr_allow_enfield'] = 'Lee Enfield';
$lang['scr_allow_kar98ksniper'] = 'Scoped Kar98k';
$lang['scr_allow_nagantsniper'] = 'Scoped Mosin-Nagant';
$lang['scr_allow_enfieldsniper'] = 'Scoped Lee Enfield';
$lang['scr_allow_springfield'] = 'Springfield';


$commands_disabled[] = 'rcon_password';
$commands_disabled[] = 'dvardump';
$commands_disabled[] = 'dvarlist';
$commands_disabled[] = 'setfromdvar';

$corrupted_join_char_fix = true;

?>
